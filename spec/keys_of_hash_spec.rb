describe 'keys_of' do
  ...
  xit "returns red-footed tortoise in an array for the animal hash when passed the argument 'Panama'" do
    result = animals.keys_of('Panama')
    expect(result).to include("red-footed tortoise")
    expect(result.length).to eq(1)
  end

  xit "returns two species in an array for the animal hash when passed the argument 'Madagascar'" do
    result = animals.keys_of('Madagascar')
    ["aye-aye", "tomato frog"].each {|animal| expect(result).to include(animal) }
    expect(result.length).to eq(2)
  end

  xit "returns three species in an array for the animal hash when passed the argument 'Australia'" do
    result = animals.keys_of('Australia')
    ["sugar glider", "kangaroo", "koala"].each {|animal| expect(result).to include(animal) }
    expect(result.length).to eq(3)
  end

  xit 'returns the keys of the entered value' do
